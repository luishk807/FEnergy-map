...
<script type="text/javascript">
 ...
 function initialize() {
	...
	infox[0][1]='<u>".getName($mapentry[0]["userid"])."</u><br/>Sales Made:".getMapSales($mapentry[0]["userid"],$mapentry[0]["date_worked"])."<br/>Date Work: ".fixdate_comps('invdate_s',$mapentry[0]["date_worked"])."';
	infox[0][2]='';
	";
	?>
 ...
 function addPoint(event) {
  ...
  function goBack(value)
  {
	  if(value.length >0)
	  {
		  ...
		  if(value=="reset")
			  window.location.href="showmap2_edit.php?id="+value;
		  else if(value=="back")
		  	  window.location.href="showmap2.php";
   ...
   <input type="hidden" id="id" name="id" value="<?php echo $_REQUEST["id"]; ?>" />
        ...
        <div id="footer" style="text-align:center">
			...
			if(adminPrevb($user["type"]))
			{
				...
				{
				?>
                <input type="submit" value="Set Worked" />&nbsp;&nbsp;
                <?
				}
				else
				{
             ...
             <select id="mapmenu" name="mapmenu" onchange="goBack(this.value)">
            	...
            	<option value="delete">Delete Map</option>
                <option value="back">Previous Page</option>
                <option value="reset">Reset Map</option>
             </select>
            <?php
			}
			else
			{
			?><input type="button" value="Reset Map" onclick="goBack('reset')" />&nbsp;&nbsp;<input type="button" value="Back" onclick="goBack('back')" />
            <?Php
			}
			?>
            </div>
