...
if(!checkMapAccess())
{
	$_SESSION["fmapresult"]="ERROR: Map is empty, Please upload new file to proceed";
					...
					while($rows = mysql_fetch_array($result))
					{
						...
						else if($fmap["taski"]=="address" || $fmap["taski"]=="date" || $fmap["taski"]=="daterange" || $fmap["taski"]=="office")
						{
							if(sizeof($agents)>0)
							{
								...
								if(!$found)
								{
									$ag = array("name"=>stripslashes($rows["agent"]),"code"=>$rows["agent_code"],"id"=>$rows["id"]);
									$agents[]=$ag;
								}
							}
							else
       ...
       <div style="float:left"> <!--types of divs-->
            ...
            <span id="typediv_daterange" style="display:none;">
             ...
             <span id="typediv_office" style="display:none;">
                 ...
                 <?php
				$checkoffice = "no";
				$queryof = "select distinct office from file_entries where office is not null and office !=''";
				if($rof = mysql_query($queryof))
				{
					if(($numrowof = mysql_num_rows($rof))>0)
					{
						$checkoffice="yes";
						echo "<select id='offices' name='offices'>";
						echo "<option value='na'>Select Office</option>";
						while($rowof = mysql_fetch_array($rof))
						{
							echo "<option value='".stripslashes($rowof["office"])."'>".stripslashes($rowof["office"])."</option>";
						}
						echo "</select>";
					}
					else
						echo "<input type='text' id='offices' name='offices' size='30'/>&nbsp;&nbsp;&nbsp;";
				}
				else
					echo "<input type='text' id='offices' name='offices' size='30'/>&nbsp;&nbsp;&nbsp;";
				?>
                 <input type="hidden" id="checkoffice" name="checkoffice" value="<?php echo $checkoffice; ?>" />
                 &nbsp;&nbsp;&nbsp;
                	...
                	<?php
						...
						if($result = mysql_query($query))
						{
							if(($num_rows = mysql_num_rows($result))>0)
							{
								while($rows = mysql_fetch_array($result))
								{
									echo "<option value='".$rows["id"]."'>".stripslashes($rows["name"])."-".$rows["date"]."</option>";
								}
							}
						}
						?>
				  ...
				  <legend>
                    ...
                    Summary From Entry:
					 <select id="fileinfo" onchange="changemap(this.value,'fileinfo')" title="View Files From The Dates Avaliable">
						...
						if($result = mysql_query($query))
						{
							if(($num_rows = mysql_num_rows($result))>0)
							{
								...
								{
									if($fileid==$rows["id"])
										echo "<option value='".$rows["id"]."' selected='selected'>".stripslashes($rows["name"])." - ".$rows["date"]."</option>";
									else
										echo "<option value='".$rows["id"]."'>".stripslashes($rows["name"])." - ".$rows["date"]."</option>";
								}
							}
                     ...
                     <?php
						...
						{
							?>
                    <div class='sbutton'><a  href='editimport.php?id=<?php echo base64_encode($fileid); ?>'><img src="images/fedit.jpg" name="edit" border="0"  onmouseover="document.edit.src='images/fedit_r.jpg'" alt="Edit This File Entry" onmouseout="document.edit.src='images/fedit.jpg'" title="Edit This File Entry"/></a></div>&nbsp;&nbsp;&nbsp;
                    <?php
						}
						?>
                        <div class='sbutton'>
                        <a href='adentry.php?id=<?php echo base64_encode($fileid); ?>'><img src="images/fadd.jpg" name="aden" border="0"  onmouseover="document.aden.src='images/fadd_r.jpg'" alt="Add New Entry" onmouseout="document.aden.src='images/fadd.jpg'" title="Add New Entry"/></a></div>
                   <div style="clear:both"></div>
				</legend>
				 ...
				 if($showdiv)
				 {
				  ...
				  <div id="filesind">
					<fieldset><!--show agent descriptive-->
					  ...
					  {
						  if(!empty($agentcode))
							$query = "select * from file_entries where agent_code = '".$agentcode."' and fileid='".$fileid."'";
						  else
							$query = "select * from file_entries where agent_code = '".$agents[0]["code"]."' and fileid='".$fileid."'";
						 if($result = mysql_query($query))
						 {
							 $uadddress = array();
							 $ucode ="";
							 ...
							 if(($num_rows = mysql_num_rows($result))>1)
							 {
								...
								{
									$ucode=$row["agent_code"];
									$umang =$row["manager"];
									$utotalg +=$row["totalg"];
									$utotalp +=$row["totalp"];
									$ad = array("sad"=>$row["address1"],"ead"=>$row["address2"],"id"=>$row["id"]);								
									$uaddress[]=$ad;
								}
								$utotalgros=$utotalg+$utotalp;
							 }
							 else
							 {
								$userdata = mysql_fetch_assoc($result);
								$ad = array("sad"=>$userdata["address1"],"ead"=>$userdata["address2"],"id"=>$userdata["id"]);								
								$uaddress[]=$ad;
								$ucode=$userdata["agent_code"];
								$umang =$userdata["manager"];
								$utotalg +=$userdata["totalg"];
								$utotalp +=$userdata["totalp"];
								$utotalgros=$utotalg+$utotalp;
							 }
						 }
					  }
					 ...
					 <table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
						<td width="50%" height="38" align="left" valign="middle">Rep Code: <? echo $ucode; ?></td>
						<td width="50%" align="left" valign="middle">Manager: <?php echo $umang;   ?></td>
					  </tr>
					  <tr>
						<td height="34" colspan="2" align="left" valign="middle"><hr /></td>
					   </tr>
					  <tr>
						<td height="34" align="left" valign="middle">Total Gas: <?php echo $utotalg; ?></td>
						<td align="left" valign="middle">Total Power: <?Php echo $utotalp; ?></td>
					  </tr>
					  <tr>
						<td height="51" colspan="2" align="left" valign="middle">Total Gross Sales: <?php echo $utotalgros; ?></td>
					   </tr>
                       <tr><td height='60' colspan='2' align='left' valign='middle'>
                       <span class='importSub'>List of Address Covered</span>&nbsp;<img src='images/checkinlineb.jpg' alt='line' border='0' /><br/>
                       <?Php
					   if(sizeof($uaddress)>0)
					   {
						   for($i=0;$i<sizeof($uaddress);$i++)
						   {
							   echo "
                        Start Address: <b> ".$uaddress[$i]["sad"]."</b><br/>
                        End Address: <b>".$uaddress[$i]["ead"]."</b><br/><br/><a style='font-size:15pt' href='editad.php?id=".base64_encode($uaddress[$i]["id"])."'>Wrong Address? Click Here To Edit</a><br/><img src='images/checkinlineb.jpg' alt='line' border='0' /><br/>";
						   }
					   }
					   ?>
                       </td></tr>
				 ...
				 ?>
					<?php
					  if($taskcheck=="agent_searchbar")
					  {
						  ...
						  <fieldset><!--show agent descriptive-->
					 <legend>
                     	<div style="float:left">View Performance of: 
                    ...
                    <div class='sbutton'><a  href='editad.php?id=<?php echo base64_encode($agentcode);?>'><img src="images/fedit.jpg" name="edit" border="0"  onmouseover="document.edit.src='images/fedit_r.jpg'" alt="Edit This File Entry" onmouseout="document.edit.src='images/fedit.jpg'" title="Edit This File Entry"/></a></div>
                    <div style="clear:both"></div>
					 </legend>
				  <?php
					   if(sizeof($agents)>0)
